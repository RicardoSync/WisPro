...
from bk_update import actualizar_bloqueo, actualizar_desbloqueo, actualizar_cancelacion, actualizar_suspencion
...
def suspender_ip(host, user, password, ip_bloqueo, id):
    try:
        # Iniciar conexión SSH
        client = paramiko.SSHClient()
        client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ...
        bloqueo = actualizar_suspencion(id)
        if bloqueo:
            # Cerrar conexión
            client.close()
            return True
...
def cancelar_ip(host, user, password, ip_bloqueo, id):
    try:
        # Iniciar conexión SSH
        client = paramiko.SSHClient()
        client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ...
        bloqueo = actualizar_cancelacion(id)
        if bloqueo:
            # Cerrar conexión
            client.close()
            return True
