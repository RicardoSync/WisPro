...
from bk_bloquear_cliente import cancelar_ip, suspender_ip
...
def get_datos(id, microtik, windows):
    ...
    if ip_cliente:
        if ip_microtik:
            if suspender_ip(host=ip_microtik[0], user=ip_microtik[1], password=ip_microtik[2], ip_bloqueo=ip_cliente[0], id=id):
                windows.destroy()
                messagebox.showinfo("SpiderNet", "El cliente se suspendio con exito")
        else:
            windows.destroy()
...
def get_desbloqueo(id, microtik, windows):
    ...
    if ip_cliente:
        if ip_microtik:
            if cancelar_ip(host=ip_microtik[0], user=ip_microtik[1], password=ip_microtik[2], ip_bloqueo=ip_cliente[0], id=id):
                windows.destroy()
                messagebox.showinfo("SpiderNet", "El cliente se cancelo con exito")
        else:
            windows.destroy()
            messagebox.showerror("SpiderNet", "Hubo un error, verifica los datos")


def md_panel_suspencion_cancelacion(id, nombreCliente):
    ...
    btnBloquear = CTkButton(windows, border_color=colores["marcos"], border_width=2, corner_radius=6,
                            fg_color="red", text="Suspender", text_color="black",
                            width=250,
    ...
    btnDesbloquear = CTkButton(windows, border_color=colores["marcos"], border_width=2, corner_radius=6,
                            fg_color="blue", text="Cancelar", text_color="white",
                            width=250,
